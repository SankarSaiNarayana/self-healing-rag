...
def _bm25_retrieve(collection: str, question: str, k: int) -> list[RetrievedChunk]:
    ...
    scores = bm25.get_scores(toks) if rows else []
    if not scores:
        return []
...
def _rerank(question: str, chunks: list[RetrievedChunk]) -> list[RetrievedChunk]:
    ...
    return [
        RetrievedChunk(doc_id=c.doc_id, chunk_id=c.chunk_id, source=c.source, score=float(s), text=c.text)
...
def _hybrid_merge(
    *,
    ...
    for cid, row in merged.items():
        ...
        out.append(RetrievedChunk(doc_id=c.doc_id, chunk_id=cid, source=c.source, score=float(score), text=c.text))
...
def retrieve(
    *,
    ...
    return RetrievalResult(
        query=q,
        chunks=merged[: max(top_k, bm25_k, 1)],
...
def retrieve_with_self_heal(
    *,
    ...
    for attempt in range(settings.max_retrieval_retries + 1):
        ...
        res = retrieve(question=q2, collection=collection, query_type=query_type, top_k=tk, bm25_k=bk, rerank=rerank)
        last = res
